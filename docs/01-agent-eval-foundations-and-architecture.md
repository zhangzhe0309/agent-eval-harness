...
写 Agent 评测之前，先说句实情：**别把 Agent 当普通的 LLM Chatbot 来测。**

Chatbot 答错了最多是一句废话，而 Agent 一旦有了工具调用权限，是真会去连数据库改数据、调 API 扣款、甚至在 Linux 服务器上删错日志的。

它的核心流程很直接：
```
[用户需求/事件] ──> [LLM 思考 Thought] ──> [调用工具 Action] ──> [读取环境 Observation]
       ▲                                                                      │
       └──────────────────���───── (循环直到任务结束) ───────────────────────────┘
```
